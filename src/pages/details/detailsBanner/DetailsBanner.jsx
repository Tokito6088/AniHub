						...
						<React.Fragment>
							...
							<ContentWrapper>
								<div className='content'>
									<div className='left'>{item?.data?.images.jpg.large_image_url ? <Img className='posterImg' src={item?.data?.images.jpg.large_image_url} /> : <Img className='posterImg' src={PosterFallback} />}</div>
									<div className='right'>
										<div className='title'>{`${item?.data?.title_english ? item?.data?.title_english : item?.data?.title}`}</div>
										<div className='subtitle'>{item?.data?.title}</div>
										<Genres data={_genres} />

										<div className='row'>{item?.data?.score ? <CircleRating rating={item?.data?.score.toFixed(1)} /> : ''}</div>
